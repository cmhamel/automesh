...
impl Tree for QuadTree {
    ...
    fn sandbox(&mut self, levels: &usize) -> (Connectivity, Points) {
        ...
        let maps = (0..*levels).rev().map(|level|
            self.iter().enumerate().filter(|(_, cell)|
                cell.get_level() == &level
            ).map(|(index, _)|
                index
            ).collect()
    ...
    fn template(&self, cell: &Cell, node: &mut usize) -> (Template, Points) {
        if let Some(cells) = cell.cells {
            ...
            let foo: Vec<bool> = cells.iter().map(|&subcell|
                self[subcell].cells.is_some()
            ).collect();
            let node_0 = *node;
            if !foo[0] && !foo[1] && !foo[2] && !foo[3] {
                *node += 4;
                ...
                let nodes = match &self[cells[3]].template {
                    Some(Template::T0000(connectivity)) => {
                        connectivity[0]
                    }
                    _ => panic!()
                };
...
fn foo() {
    ...
    nodal_coordinates.iter().enumerate().for_each(|(node, coord)| {
        file.write_all(format!("\t{}", node + 1).as_bytes()).unwrap();
        coord.iter().for_each(|entry|
            file.write_all(format!(",\t{}", entry).as_bytes()).unwrap()
        );
        file.write_all("\n".as_bytes()).unwrap()
    });
